  ...
  public final static class Loader {
    ...
    void loadINodeDirectorySection(InputStream in, final Configuration conf)
      ...
      final List<INodeReference> refList = parent.getLoaderContext()
          .getRefList();
      PipelineTask task = new PipelineTask(conf, DFSConfigKeys.DFS_NAMENODE_IMAGELOAD_KEY_PREFIX);
      if (conf.get(task.getBlockingThresholdKey("INodeDirectorySectionLoader")) == null) {
        conf.setInt(task.getBlockingThresholdKey("INodeDirectorySectionLoader"), 20);
      }
      if (conf.get(task.getTransferThresholdKey("INodeDirectorySectionLoader")) == null) {
        conf.setInt(task.getTransferThresholdKey("INodeDirectorySectionLoader"), 10);
      }
      if (conf.get(task.getBlockingThresholdKey("DirectoryInitializer")) == null) {
        conf.setInt(task.getBlockingThresholdKey("DirectoryInitializer"), 1000);
      }
      if (conf.get(task.getTransferThresholdKey("DirectoryInitializer")) == null) {
      ...
      }, "INodeDirectorySectionLoader").appendWork(
          new PipelineWork<INodeDirectorySection.DirEntry, Void>() {
            @Override
    ...
    void loadINodeSection(InputStream in, StartupProgress prog,
      ...
      */
      PipelineTask task = new PipelineTask(conf, DFSConfigKeys.DFS_NAMENODE_IMAGELOAD_KEY_PREFIX);
      task =
          task.appendWork(new PipelineWork<Void, INodeSection.INode>() {
            int readed = 0;
            @Override
            public INodeSection.INode doWork(Void obj) throws IOException {
              do {
                if (readed < s.getNumInodes()) {
                  readed++;
                  INodeSection.INode p =
                      INodeSection.INode.parseDelimitedFrom(in);
                  if (p.getId() == INodeId.ROOT_INODE_ID) {
                    loadRootINode(p);
                  } else {
                    return p;
              ...
              .appendWork(new PipelineWork<INode, Void>() {
                @Override
